...
cp ~/PublicSMB/peaq.qa.test/parachain.raw.1.config data/genesis.json
# 2. Put the QA's fork.json file in the data/fork.json
cp ~/PublicSMB/peaq.qa.test/parachain.raw.1.config data/fork.json
# 3. Put the QA's runtime module
cp ~/PublicSMB/peaq.qa.test/parachain.1.wasm data/runtime.wasm

HTTP_RPC_ENDPOINT=http://localhost:10033 scripts/docker-start.sh
